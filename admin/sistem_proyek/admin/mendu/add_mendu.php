<div class="card card-primary">
	...
	<form action="" method="post" enctype="multipart/form-data">
		<div class="card-body">
			...
			<div class="form-group row">
				<label class="col-sm-2 col-form-label">File Surat Keterangan </label>
				<div class="col-sm-6">
					<input type="file" class="form-control" id="surat_keterangan" name="surat_keterangan"  required="required">
...
<?php
    ...
    //mulai proses simpan data
	    $nomor    = $_POST['surat_keterangan'];
        $ekstensi_diperbolehkan    = array('jpg','png','jpeg','pdf');
        $nama    = $_FILES['surat_keterangan']['name'];
        $x        = explode('.', $nama);
        $ekstensi    = strtolower(end($x));
        $ukuran        = $_FILES['surat_keterangan']['size'];
        $file_tmp    = $_FILES['surat_keterangan']['tmp_name']; 
     
        if(in_array($ekstensi, $ekstensi_diperbolehkan) === true){
        move_uploaded_file($file_tmp, 'C:/xampp/htdocs/sistem_penduduk/admin/mendu/images/'.$nama);
				
        $sql_simpan = "INSERT INTO tb_mendu (id_pdd, tgl_mendu, sebab,file_sk) VALUES (
			'".$_POST['id_pdd']."',
            '".$_POST['tgl_mendu']."',
			'".$_POST['sebab']."',
            '$nama')";
