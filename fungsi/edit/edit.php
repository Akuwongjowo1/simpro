...
session_start();
    ...
    $catatan = htmlentities($_POST['ket']); // Mengambil catatan dari input
    ...
    if ($hasil['stok'] > $jumlah) {
        ...
        $data1[] = $jumlah;
        $data1[] = $total;
        $data1[] = $catatan; // Menambahkan catatan ke dalam array data
        $data1[] = $id;

        $sql1 = 'UPDATE penjualan SET jumlah=?, total=?, catatan=? WHERE id_penjualan=?'; // Memperbarui query untuk mencakup kolom catatan
        $row1 = $config->prepare($sql1);
        $row1->execute($data1);
        echo '<script>window.location="../../index.php?page=add-pemesanan#keranjang"</script>';
    } else {
   ...
   if (!empty($_GET['cari_barang'])) {
    ...
    if ($cari == '') {
        // If search keyword is empty, fetch all product data
        $sql = "SELECT * from barang";
        $row = $config->prepare($sql);
        $row->execute();
        $hasil1 = $row->fetchAll();
    } else {
        // If search keyword is not empty, perform search query
